    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3 items-center">
          <label className="flex items-center gap-2 cursor-pointer font-bold text-sm bg-white p-2 rounded-xl border-2 border-[var(--ink)]">
            <input 
              type="checkbox" 
              checked={blindMode} 
      ...
      {/* Search Bar */}
      <NeuCard className="p-4 bg-white flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            className="neu-input pl-11 py-4 text-lg" 
            placeholder="Describe the candidate you need (e.g., 'React developer with system design skills and good communication')"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>
        <NeuButton variant="primary" className="py-4 px-8 text-lg flex items-center gap-2">
          <FaMagic /> AI Search
        </NeuButton>
      </NeuCard>
      ...
      <div className="grid md:grid-cols-4 gap-8">
        ...
        <div className="space-y-6">
          <NeuCard className="p-6 bg-white">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2 border-b-[3px] border-[var(--ink)] pb-2"><FaFilter /> Filters</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="text-xs font-bold bg-[var(--acid)] border-2 border-[var(--ink)] px-2 py-1 rounded-full">React</span>
                  <span className="text-xs font-bold bg-[var(--mint)] border-2 border-[var(--ink)] px-2 py-1 rounded-full">Node.js</span>
                  <span className="text-xs font-bold bg-[var(--sky)] border-2 border-[var(--ink)] px-2 py-1 rounded-full">MongoDB</span>
                </div>
              </div>
            </div>
          </NeuCard>
        ...
        <div className="md:col-span-3 space-y-4">
          <div className="flex justify-between items-center bg-[#f8f7f4] p-4 rounded-xl border-2 border-[var(--ink)]">
            <span className="font-bold">3 Top Matches Found</span>
            <select className="neu-select py-1 px-3 w-auto text-sm">
          ...
          {[
            ...
            <NeuCard key={i} className="p-6 bg-white hover:border-[var(--electric)] transition-colors cursor-pointer">
              <div className="flex flex-col md:flex-row justify-between gap-6">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {candidate.skills.map(s => (
                      <span key={s} className="bg-[var(--paper)] border border-[var(--ink)] px-2 py-1 rounded-md text-xs font-bold">{s}</span>
                    ))}
                    <span className="bg-gray-100 border border-gray-300 text-gray-500 px-2 py-1 rounded-md text-xs font-bold">+12 more</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end justify-between min-w-[120px]">
                  <div className="text-right mb-4">
                    ...
                    <p className="text-3xl font-bold text-[var(--electric)]">{candidate.match}%</p>
                  </div>
