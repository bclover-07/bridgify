...
export default function StudentDashboard() {
  ...
  const stats = [
    { label: 'Total Skills', value: data?.stats?.totalSkills || 0, icon: FiTarget, href: '/student/profile', bg: '#4B3AFF' },
    { label: 'Evidence Points', value: data?.stats?.totalEvidence || 0, icon: FiAward, href: '/student/profile', bg: '#2FE3A3' },
    { label: 'Avg Confidence', value: `${data?.stats?.avgConfidence || 0}%`, icon: FiTrendingUp, href: '/student/benchmarks', bg: '#FF3D9A' },
    { label: 'Assessments', value: data?.upcomingAssessments?.length || 0, icon: FiClipboard, href: '/student/assignments', bg: '#FFB020' },
  ];

  const radarData = data?.segSummary?.slice(0, 6).map(s => ({
    name: s.skillLabel?.substring(0, 14) || 'Skill',
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold mb-1">
              Hey, {data?.profile?.name?.split(' ')[0] || 'Arjun'}! 🎓
            </h1>
        ...
        {stats.map((s, i) => (
          <Link key={i} href={s.href}>
            <motion.div whileHover={{ y: -4, scale: 1.02 }} className="transition-all cursor-pointer">
              <NeuCard className="p-4 md:p-5 bg-white hover:border-[var(--electric)]">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <StaggerItem>
          <NeuCard className="p-5 bg-white">
            ...
            {radarData.length >= 3 ? (
              <NeuRadarChart data={radarData} angleKey="name" dataKeys={[{ key: 'score', label: 'Confidence', color: '#4B3AFF' }]} height={280} />
            ) : (
            ...
            {data?.upcomingAssessments?.length > 0 ? (
              <div className="space-y-3">
                {data.upcomingAssessments.slice(0, 5).map((a, i) => (
                  <Link key={a._id || i} href="/student/assignments">
